...
from arclet.entari import At, ChannelType, Session, plugin, metadata
            ...
            if session.account.platform == "qq":
                await session.account.protocol.send_message(
                    ...
                    [
                        ...
                        Button.input("/功能 禁用 member_leave")("取消退群提醒"),
                    ],
        ...
        if channel.type is ChannelType.TEXT:
            if session.account.platform == "qq":
                await session.account.protocol.send_message(
                    ...
                    [
                        "欢迎新成员！",
                        At(session.event.user.id),
                        "进群了就别想跑哦~",
                        Button.input("/功能 禁用 member_join")("取消入群提醒"),
