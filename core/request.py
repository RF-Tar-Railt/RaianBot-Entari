...
async def on_friend_request(event: FriendRequestEvent, session: Session, app: Entari):
    ...
    if superusers:
        ...
        await session.account.send_message(channel, f"收到添加好友请求：\n{event.user}")

        async def waiter(sess: Session[MessageCreatedEvent]):
            if sess.event.channel.id == channel.id:
                return sess.content

        await session.account.send_message(channel, "处理请求等待中")
        ...
        if not resp:
            ...
            await session.account.send_message(channel, "处理请求超时")
            return
        if resp in ("同意", "yes", "y", "ok", "好", "是", "同意请求"):
            await session.request_approve(True)
            await session.account.send_message(channel, "已同意好友请求")
            return
        await session.request_approve(False, "管理员拒绝了请求")
        await session.account.send_message(channel, "已拒绝好友请求")
...
async def on_guild_request(event: GuildRequestEvent, session: Session, app: Entari):
    ...
    if superusers:
        ...
        if channel_feedback:
            await session.account.send_message(channel_feedback, "请等待机器人管理员处理入群请求")
        await session.account.send_message(channel_send, "处理请求等待中")
        ...
        if not resp:
            await session.request_approve(False, "管理员超时未回复，请尝试重新发送请求")
            await session.account.send_message(channel_send, "处理请求超时")
            if channel_feedback:
                await session.account.send_message(channel_feedback, "管理员超时未回复，请尝试重新发送请求")
            return
        if resp in ("同意", "yes", "y", "ok", "好", "是", "同意请求"):
            await session.request_approve(True)
            await session.account.send_message(channel_send, "已同意入群请求")
            if channel_feedback:
                await session.account.send_message(channel_feedback, "管理员已同意入群请求")
            return
        await session.request_approve(False, "管理员拒绝了请求")
        await session.account.send_message(channel_send, "已拒绝入群请求")
        if channel_feedback:
            await session.account.send_message(channel_feedback, "管理员已拒绝入群请求")
