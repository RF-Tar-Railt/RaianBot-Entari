...
from .model import WeiboPost, WeiboUser
from .cache import WeiboUserCache
...
class WeiboAPI:
    ...
    async def _call(self, params: dict, timeout: int = 10) -> dict[str, Any]:
        ...
        headers = {
            "Host": "m.weibo.cn",
            "Referer": f"{base_url}{'&'.join(f'{k}={v}' for k, v in params.items())}",
    ...
    async def get_profile(
        ...
        if cache and (exist := self.cache.get(uid)):
            return exist
    ...
    async def get_profile_by_name(
        ...
        index = max(index, 0)
        if cache:
            for user in self.cache.data.values():
                if user.name == name:
                    return user
    ...
    async def get_profiles(self, name: str) -> list[WeiboUser]:
        ...
        for i in await self.search_users(name):
            try:
                res.append(await self.get_profile(i, save=True))
            except RespDataError:
                continue
        return res

    def _handler_dynamic(self, data: dict) -> WeiboPost:
        text = "".join(elem.dumps(strip=True) for elem in parse(data["text"]))
        dynamic = WeiboPost(bid=data["bid"], text=text)
        ...
        if (page_info := data.get("page_info")) and page_info["type"] == "video":
            ...
            dynamic.video_url = page_info["page_url"]
    ...
    async def get_dynamic(
        ...
        if save:
            ...
            await self.cache.merge(target)
        return res

    async def update(self, target: int) -> WeiboPost | None:
        ...
        if d_data["cardlistInfo"]["total"] > dynamic_total:
            ...
            await self.cache.merge(profile)
            return dy
        return
